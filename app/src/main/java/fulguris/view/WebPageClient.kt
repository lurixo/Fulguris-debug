...
//    private var elementHide = userPreferences.elementHide
    ...
    override fun shouldInterceptRequest(view: WebView, request: WebResourceRequest): WebResourceResponse? {
        ...
        // === Captcha Debug: log every request ===
        fulguris.debug.CaptchaDebugLogger.logRequest(request, request.isForMainFrame)
        ...
        // === Captcha Debug: log blocked requests ===
        if (wasBlocked) {
            fulguris.debug.CaptchaDebugLogger.logBlockedRequest(url, request.isForMainFrame)
        }
...
//        }
        ...
        if (engine != null) {
            ...
            if (engineResponse != null) {
                ...
                // === Captcha Debug: log engine handling ===
                fulguris.debug.CaptchaDebugLogger.logNetworkEngineRequest(request.url.toString(), engine.displayName)
                fulguris.debug.CaptchaDebugLogger.logResponse(request.url.toString(), engineResponse)
    ...
    override fun onPageFinished(view: WebView, url: String) {
        ...
        // === Captcha Debug: log page finish with full state for Google pages ===
        fulguris.debug.CaptchaDebugLogger.logPageEvent("onPageFinished", url,
            "LoadTime: ${pageLoadDuration}ms, Resources: $iResourceCount")
        fulguris.debug.CaptchaDebugLogger.logCookies(url)
        val isGooglePage = url.contains("google.com") || url.contains("recaptcha")
                || url.contains("gstatic.com") || url.contains("accounts.google")
        if (isGooglePage) {
            fulguris.debug.CaptchaDebugLogger.dumpFullSnapshot(view, webPageTab.requestHeaders, url)
            fulguris.debug.CaptchaDebugLogger.injectFingerprintCheck(view)
        }
...
/*        // TODO: element hiding does not work
    ...
    override fun onPageStarted(view: WebView, url: String, favicon: Bitmap?) {
        ...
        // === Captcha Debug: log page start ===
        fulguris.debug.CaptchaDebugLogger.logPageEvent("onPageStarted", url)
